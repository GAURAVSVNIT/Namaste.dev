...
# from schp.model import SCHP  # type: ignore
...
class AutoMasker:
    def __init__(
        ...
        # Temporarily commented out SCHP for testing
        ...
        #     device=device,
        # )
        self.schp_processor_atr = None
        self.schp_processor_lip = None
    ...
    def process_schp_lip(self, image_or_path):
        if self.schp_processor_lip is None:
            # Return a dummy mask for testing
            from PIL import Image
            import numpy as np
            if isinstance(image_or_path, str):
                ...
                img = image_or_path
            return Image.fromarray(np.zeros((img.height, img.width), dtype=np.uint8))
        return self.schp_processor_lip(image_or_path)

    def process_schp_atr(self, image_or_path):
        if self.schp_processor_atr is None:
            # Return a dummy mask for testing
            from PIL import Image
            import numpy as np
            if isinstance(image_or_path, str):
    ...
    def preprocess_image(self, image_or_path):
        return {
            ...
            "schp_atr": self.process_schp_atr(image_or_path),
            "schp_lip": self.process_schp_lip(image_or_path),
        }
